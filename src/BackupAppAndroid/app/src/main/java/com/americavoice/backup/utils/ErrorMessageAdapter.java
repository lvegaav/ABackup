...
import com.americavoice.backup.operations.CreateFolderOperation;
        ...
        if (!result.isSuccess() && isCommonError(result.getCode())) {
            ...
            if (result.isSuccess()) {
                message = String.format(
                        res.getString(R.string.files_uploader_upload_succeeded_content_single),
                ...
                if (result.getCode() == ResultCode.LOCAL_STORAGE_FULL
                    ...
                    message = String.format(
                            res.getString(R.string.files_error__upload__local_file_not_copied),
                            ((UploadFileOperation) operation).getFileName(),
                ...
                /*
                    ...
                    */
                    ...
                    message = String.format(res.getString(R.string.files_forbidden_permissions),
                            res.getString(R.string.files_uploader_upload_forbidden_permissions));

                } else if (result.getCode() == ResultCode.INVALID_CHARACTER_DETECT_IN_SERVER) {
                    message = res.getString(R.string.files_filename_forbidden_charaters_from_server);

                } else {
                    message = String.format(
                            res.getString(R.string.files_uploader_upload_failed_content_single),
                            ((UploadFileOperation) operation).getFileName());
                }
            }
            ...
            if (result.getCode() == ResultCode.INVALID_CHARACTER_IN_NAME) {
                message = res.getString(R.string.files_filename_forbidden_characters);

            } else if (result.getCode().equals(ResultCode.FORBIDDEN)) {
                message = String.format(res.getString(R.string.files_forbidden_permissions),
                        res.getString(R.string.files_forbidden_permissions_create));

            } else if (result.getCode() == ResultCode.INVALID_CHARACTER_DETECT_IN_SERVER) {
                message = res.getString(R.string.files_filename_forbidden_charaters_from_server);

            } else {
                message = res.getString(R.string.files_create_dir_fail);
            }
    ...
    private static String getCommonErrorMessage(RemoteOperationResult result, Resources res) {
        ...
        if (!result.isSuccess()) {
            ...
            if (result.getCode() == ResultCode.WRONG_CONNECTION) {
                ...
                message = res.getString(R.string.network_server_maintenance_mode);
            }
        }
