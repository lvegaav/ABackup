...
public class AccountUtils {
        ...
        if (defaultAccount == null && ocAccounts.length > 0) {
            ...
            for (Account account: ocAccounts) {
                boolean pendingForRemoval = arbitraryDataProvider.getBooleanValue(account,
                        AuthenticationConstants.PENDING_FOR_REMOVAL);
     ...
     * @param   authTokenType   Authorization token type, matching some of the AUTH_TOKEN_TYPE_* constants.
    ...
    public static String getWebdavPath(OwnCloudVersion version, String authTokenType) {
        if (version != null) {
            if (AccountTypeUtils.getAuthTokenTypeAccessToken(BaseConstants.ACCOUNT_TYPE).equals(authTokenType)) {
                return ODAV_PATH;
            }
            if (AccountTypeUtils.getAuthTokenTypeSamlSessionCookie(BaseConstants.ACCOUNT_TYPE).equals(authTokenType)) {
    ...
    public static void updateAccountVersion(Context context) {
        ...
        if ( currentAccount != null ) {
            ...
            if (currentAccountVersion == null) {
                ...
                Account[] ocAccounts = accountMgr.getAccountsByType(BaseConstants.ACCOUNT_TYPE);
                String serverUrl;
                String username;
                ...
                for (Account account : ocAccounts) {
                    ...
                    if (!newAccountName.equals(account.name)) {
                        ...
                        newAccount = new Account(newAccountName, BaseConstants.ACCOUNT_TYPE);
     ...
     */
    public static OwnCloudVersion getServerVersion(Account account, Context context) {
    ...
    public static boolean hasSearchUsersSupport(Account account, Context context){
        OwnCloudVersion serverVersion = getServerVersion(account, context);
        return (serverVersion != null && serverVersion.isSearchUsersSupported());
    }

    public static boolean hasSearchSupport(Account account, Context context) {
        OwnCloudVersion serverVersion = getServerVersion(account, context);
